...
\newcommand{\defeq}{\colon\equiv}  % A judgmental equality currently being defined
...
\renewcommand{\map}[2]{\ensuremath{{#1}\mathopen{}\left({#2}\right)\mathclose{}}\xspace}
\let\Ap\map
...
\def\pb{}% needs fixing\save[]+<3.5mm,-3.5mm>*{\copy\pbbox} \restore}
...
%%% ePub-specific macros

%% The following are reasonable substitutions for existing macros

\let\OPTwidow=\relax
\let\leadsto\rightsquigarrow

\expandafter\let\csname narrowmultline*\expandafter\endcsname\csname multline*\endcsname
\expandafter\let\csname endnarrowmultline*\expandafter\endcsname\csname endmultline*\endcsname
\let\narrowmultline\multline
\let\endnarrowmultline\endmultline
\expandafter\let\csname flalign*\expandafter\endcsname\csname align*\endcsname
\expandafter\let\csname endflalign*\expandafter\endcsname\csname endalign*\endcsname

\newtheorem*{comment}{Comment}

\NewDocumentEnvironment {textequation} {}
{
...
\let\MoveEqLeft=\relax

\def\mapsfrom{\hexentity{021A4}}
\def\longmapsto{\hexentity{027FC}}

%\let\phantomsection=\relax
\let\narrowequation=\ensuremath
\let\qedhere=\relax
\renewcommand\lnot{\hexentity{ac}}

\let\allowdisplaybreaks=\relax

\NewDocumentEnvironment {alignat*} {m}
{
...
\def\trianglelefteqslant{\hexentity{022B4}}

%% The following need some thought on how best to replace

\let\narrowbreak\relax
\let\eqref=\ref
\let\ocircle=\odot
\let\vcentcolon=\colon

\usepackage{environ}
\NewEnviron {tikzpicture} [1][] {}{}

\DeclareDocumentEnvironment {mathpar} {}
{
  \mpbindings
...
\DeclareDocumentEnvironment {mathparpagebreakable} {}
{
  \mpbindings
...
\def\tag#1{\qquad \text{#1}}
\let\notag\relax

\NewDocumentCommand \xymatrix {l m} {\itexgroup{}}
\let\vcenter=\relax
\itexRestoreCatcodes

\def\parbox#1{}

\let\uppercurveobject\relax
\let\lowercurveobject\relax
\let\twocellhead\relax

\DeclareDocumentCommand \titleformat {m o m m m m}

\let\OPTsmalltable=\relax

\let\centering=\relax

\def\colorbox#1#2{\swallowhbox#2}
\def\swallowhbox \hbox to 20pt{}
\def\ding#1{\hexentity{2713}}

\DeclareDocumentCommand \addlinespace {o} {}

\renewcommand{\defeq}{\itexmathrel{\hexentity{02236}\hexentity{02261}}}

\let\OPTindexfont=\relax
\crefname{indexanchor}{}{}

\let\itexmathbin=\mathbin
\let\itexmathrel=\mathrel
\def\mathbin#1{\:{#1}\:}
\def\mathrel#1{\;{#1}\;}
